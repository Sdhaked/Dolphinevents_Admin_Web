...
function createPagination(totalPages, page) {
  ...
  if (page > 2) {
    //if page value is greater than 2, add 1 after the previous button
    liTag += `
    ...
    `;
    if (page > 3) {
      //if page value is greater than 3, add (...) after the first li or page
      liTag += `
  ...
  for (let plength = beforePage; plength <= afterPage; plength++) {
    if (plength > totalPages) {
      // if plength is greater than totalPage length, continue
      continue;
    }
    if (plength === 0) {
      // if plength is 0, add +1 to plength value
      plength = plength + 1;
    }
  ...
  if (page < totalPages - 1) {
    // if page value is less than totalPage value by -1, show the last li or page
    if (page < totalPages - 2) {
      // if page value is less than totalPage value by -2, add (...) before the last li or page
      liTag += `
        <li class="dots"><span>...</span></li>
      `;
    }
  ...
  if (page < totalPages) {
    // show the next button if the page value is less than totalPages
    liTag += `
