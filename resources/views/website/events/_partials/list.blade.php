            ...
            <div class="event-archive-card">
                ...
                <div class="text-holder">
                    ...
                    <p class="date-time">
                        ...
                        <time datetime="{{ $event?->from_date->format('Y-m-d') }}">
                            {{ $event?->from_date->format('M j, Y') }}
                        </time>
                        @if ($event?->to_date)
                            <span>- <time datetime="{{ $event?->to_date->format('Y-m-d') }}">
                                    {{ $event?->to_date->format('M j, Y') }}
                                </time></span>
                        @endif
                    </p>
                    <p class="date-time">
                        ...
                        @if ($event?->to_time)
                            <span>TO <time>{{ $event?->to_time->format('g:i A') }}</time></span>
                        @endif
                    </p>
                </div>
            </div>
