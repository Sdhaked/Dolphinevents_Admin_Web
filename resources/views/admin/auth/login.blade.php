    ...
    <section class="auth-wrapper">
        <div style="width:100%; max-width: 30rem;">
            <div class="style-box auth-box">
                ...
                @if ($showOtpForm)
                    <form class="needs-validation auth-form" id="loginForm" action="{{ route('login.verify.otp') }}" method="POST" novalidate>
                        @csrf
                    ...
                    <form action="{{ route('login.change.email') }}" method="POST" class="needs-validation auth-change-email-form d-none" id="changeEmailForm" novalidate>
                        @csrf
                        <div class="form-floating mb-1">
                            <input type="email" name="email" class="form-control @error('email') is-invalid @enderror" id="changeEmail"
                ...
                @else
                    <form class="needs-validation auth-form" id="loginForm" action="{{ route('login.post') }}" method="POST" novalidate>
                        @csrf
                        <div class="form-floating mb-1">
                            <input type="email" name="email" class="form-control @error('email') is-invalid @enderror" id="myemail"
                                   value="{{ old('email', $otpEmail) }}" required>
