...
@section('body')
    ...
    <section class="wrapper">
        <main class="dash-content">
           ...
           <form action="{{ route('admin.ticket.types.store') }}" method="POST" enctype="multipart/form-data" class="needs-validation" novalidate id="createTicketTypeSeatForm">
            @csrf
            ...
            <x-admin.create-stadium />
                ...
                <div class="grid-2 grid-sm-1 gap-card">
                    <!-- Color -->
                    <div class="form-floating">
                        <input type="color" class="form-control" name="ticket_type_color" id="ticketcolor" required>
                ...
                <!-- Enable Bulk Ticket Discount -->
                <div>
                    <button type="button" class="check-btn" id="bulkDiscountToggle">
                ...
                <div>
                    <button type="submit" class="btn-md btn-sec" id="createTicketTypeSeatSubmitBtn">Submit</button>
                </div>
            </form>
            @if ($errors->any())
            ...
            <div class="style-box d-none">
                <button type="button" class="btn-xs btn-sec" data-bs-toggle="modal" data-bs-target="#modalBulkID" id="addBulkDiscountBtn"><i
    ...
    <!--Add Bulk Discount Modal Box -->
    <div class="modal fade" id="modalBulkID" tabindex="-1" aria-labelledby="modalBulkIDLabel" aria-hidden="true">
                        ...
                        <div class="input-group">
                            <div class="form-floating flex-grow-1">
                                <input type="number" name="discount_percentage" class="form-control discount-input"
                                    id="createDiscountPercentage" step="0.01" min="0" max="100" required>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="editBulkDiscountForm" class="needs-validation" novalidate>
                    <input type="hidden" id="editRowIndex">

                    <div class="form-floating mb-3">
                        <input type="number" id="editMinOrderQty" class="form-control" required>
                        <label for="editMinOrderQty">Min Order Qty*</label>
                        <div class="invalid-feedback editMinQtyErr"></div>
                    </div>

                    <div class="input-group mb-3">
                        <div class="form-floating flex-grow-1">
                            <input type="number" id="editDiscountPercentage" class="form-control" required>
    ...
    <div class="modal fade" id="deleteBulkDiscountSeatRowModal" tabindex="-1" aria-labelledby="deleteBulkDiscountSeatRowModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn-xs danger-fill-btn" id="confirmDeleteBulkDiscountSeatRowBtn">Delete</button>
                </div>
            </div>
        </div>
    </div>
...
const deleteBulkDiscountSeatRowModalElement = document.getElementById('deleteBulkDiscountSeatRowModal');
const confirmDeleteBulkDiscountSeatRowBtn = document.getElementById('confirmDeleteBulkDiscountSeatRowBtn');
let pendingBulkDiscountSeatRow = null;
...
document.getElementById("createBulkDiscountForm").addEventListener("submit", function(e) {
    ...
    bootstrap.Modal.getInstance(document.getElementById('modalBulkID')).hide();
    this.reset();
...
tbody.addEventListener("click", function (e) {
    ...
    pendingBulkDiscountSeatRow = deleteBtn.closest("tr");

    const modal = new bootstrap.Modal(deleteBulkDiscountSeatRowModalElement);
    modal.show();
});
...
tbody.addEventListener("click", function(e) {
    if (e.target.closest(".editBtn")) {
        ...
        document.getElementById("editMinOrderQty").value =
...
document.getElementById("editBulkDiscountForm").addEventListener("submit", function (e) {
    ...
    let newQty = document.getElementById("editMinOrderQty").value;
...
confirmDeleteBulkDiscountSeatRowBtn.addEventListener('click', function() {
    if (!pendingBulkDiscountSeatRow) return;

    pendingBulkDiscountSeatRow.remove();
    updateSerialNumbers();

    const modalInstance = bootstrap.Modal.getInstance(deleteBulkDiscountSeatRowModalElement);
    if (modalInstance) {
        modalInstance.hide();
    }

    pendingBulkDiscountSeatRow = null;
});

const createTicketTypeSeatForm = document.getElementById('createTicketTypeSeatForm');
const createTicketTypeSeatSubmitBtn = document.getElementById('createTicketTypeSeatSubmitBtn');
const actionLoader = document.getElementById('actionLoader');

if (createTicketTypeSeatForm) {
    createTicketTypeSeatForm.addEventListener('submit', function(event) {
        if (!createTicketTypeSeatForm.checkValidity()) {
            return;
        }

        if (createTicketTypeSeatSubmitBtn) {
            createTicketTypeSeatSubmitBtn.disabled = true;
        }
